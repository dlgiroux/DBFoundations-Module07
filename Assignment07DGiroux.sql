...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_DGiroux')
	 Begin 
	  Alter Database [Assignment07DB_DGiroux] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_DGiroux;
	 End
	Create Database Assignment07DB_DGiroux;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_DGiroux;
...
Order by ProductName, cast(DatePart(mm, InventoryDate) as varchar(50))
go
...
JOIN vInventories as I 
ON P.ProductID = I.ProductID
WHERE P.ProductID = I.InventoryID
go
*/
...
SELECT
    ...
    , [PreviousMonthCount] = IIF(Month(InventoryDate) = 1, 0, Lag(Count) OVER (Order By ProductName, Month(InventoryDate)))
FROM vProductInventories
...
AS
   RETURN 
    ...
    , PreviousMonthCount
    , CountVsPreviousCountKPI
FROM vProductInventoriesWithPreviousMonthCountsWithKPIs as vp
WHERE vp.CountVsPreviousCountKPI = @KPI)
go
